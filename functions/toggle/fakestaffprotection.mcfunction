...
execute @s[tag=ownertag,scores={SSM=1}] ~~~ tellraw @a[tag=stafftag] {"rawtext":[{"text":"§¶§cRebirth §¶§b→ §6Fake Staff Protection §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
#turn off
...
execute @s[tag=ownertag,scores={SSM=2}] ~~~ tellraw @a[tag=stafftag] {"rawtext":[{"text":"§¶§cRebirth §¶§b→ §6Fake Staff Protection §bhas been toggled §cOFF §bby §d"},{"selector":"@s"}]}
execute @s[tag=ownertag,scores={SSM=2}] ~~~ scoreboard players set ssmtoggledummy SSM 0
#Deny Nonstaff
execute @s[tag=!ownertag] ~~~ tellraw @s {"rawtext":[{"text":"§¶§cRebirth §¶§b→ This is ment for owners who have more knowledge with add-ons.\nPreform the Staff Key, then the Ownerstatus Key to unlock this command.\nWhen this is enabled, you must execute the Staff key onto your staff so they may have the stafftag tag.\nFor More info please join our Discord"}]}
execute @s[tag=!ownertag] ~~~ function rebirth/asset/discord
