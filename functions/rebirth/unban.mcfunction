execute @s[tag=!PermBan] ~~~ tellraw @a {"rawtext":[{"text":"§¶§cRebirth → §d"},{"selector":"@s"},{"text":" §¶§bWas §2unbanned §bby an §dOperator"}]}
execute @s[tag=PermBan] ~~~ tellraw @a {"rawtext":[{"text":"§¶§cRebirth → §d"},{"selector":"@s"},{"text":" §¶§cis Rebirth Global Banned and can't be unbanned"}]}
...
execute @s ~~~ function rebirth/warnreset
tag @s remove BanCBE
tag @s remove Ban
