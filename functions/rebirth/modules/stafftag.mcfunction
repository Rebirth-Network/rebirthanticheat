#Fake staff flag
execute @s[scores={ssmtoggle=1}] ~~~ execute @s[scores={I2IO2NO=!273},tag=stafftag] ~~~ tellraw @a {"rawtext":[{"text":"§¶§cRebirth → §6Fake Staff Protection : §bFlagged §d"},{"selector":"@s"},{"text":"§¶§b for unauthorized staff tag."}]}
...
execute @s[scores={ssmtoggle=1,I2IO2NO=!273}] ~~~ tag @s remove stafftag

#Removes Tags From Non Staff Members
tag @s[tag=!stafftag] remove spectate
tag @s[tag=!stafftag] remove godmode
