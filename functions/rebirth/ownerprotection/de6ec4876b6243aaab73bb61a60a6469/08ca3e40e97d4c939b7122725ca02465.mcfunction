...
execute @s[tag=stafftag,scores={SSDEBUG2=1}] ~~~ execute @s[tag=stafftag,scores={staff=1}] ~~~ tellraw @a[scores={9NW0d9Nas=7102}] {"rawtext":[{"text":"§¶§cRebirth → §bOwner status §2given §bsuccessfully to §d"},{"selector":"@s"}]}
...
execute @s[tag=!stafftag,scores={SSDEBUG2=2}] ~~~ tellraw @a[tag=stafftag] {"rawtext":[{"text":"§¶§cRebirth → §bOwner status §cremoved §bsuccessfully from §d"},{"selector":"@s"}]}
