...
const registerInformation = {
    cancelMessage: true,
    name: 'worldspawn',
    description: 'Configure the World Spawn in Rebirth',
    usage: '[ set | remove ]',
    example: [
        'worldspawn remove',
...
if(Server.player.hasTag('stafftag', chatmsg.sender.nameTag)) {
    if(worldset.includes(args[0])) {    
        ...
        Server.broadcast(`§¶§cRebirth → §b§lWorld Spawn configured to §e${Server.player.getScore('Worldx', chatmsg.sender.nameTag)} ${Server.player.getScore('Worldy', chatmsg.sender.nameTag)} ${Server.player.getScore('Worldz', chatmsg.sender.nameTag)}§b! Players will be sent here after passing World Border`, chatmsg.sender.nameTag);
    }
        ...
        Server.broadcast(`§¶§cRebirth → §b§lCustom World Spawn has been set back to default`, chatmsg.sender.nameTag);
    }
    else {
        Server.broadcast(`§¶§cRebirth → §cERROR 2! §6Usage Example §7:§b§l !worldspawn [ set | remove ]`, chatmsg.sender.nameTag);
    }
}
else {
    Server.broadcast(`§¶§cRebirth → §c§lError 4: Only Staff can configure world spawn`, chatmsg.sender.nameTag);
}
});