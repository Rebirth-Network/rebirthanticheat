...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    if( Server.player.getScore('icmtoggle', chatmsg.sender.nameTag) === 0) {
        ...
        if (!args.length || listOptions.includes(args[0])) {
            ...
            return Server.broadcast(`${allHomes ? `§l§¶§cRebirth → §6Homes §7: §b§l${allHomes.length}\n§¶§cRebirth → §6Home Names §7: §b§l${allHomes.join('§b§l, §b§l')}` : '§¶§cRebirth → §c§lNo Homes Set'}`, chatmsg.sender.nameTag);
        }
        else if (setOptions.includes(args[0])) {
            if (!args[1])
                return Server.broadcast('§¶§cRebirth → §cPlease type a UNIQUE home name to set!', chatmsg.sender.nameTag);
            if (homeName.match(coordFormat))
                return Server.broadcast('§¶§cRebirth → §cYou may not indentify your home name in a coordinate format!', chatmsg.sender.nameTag);
            if (data.statusMessage.match(homeRegex))
                return Server.broadcast('§¶§cRebirth → §cYou already have a home set with that name!', chatmsg.sender.nameTag);
            ...
            return Server.broadcast(`§¶§cRebirth → §bYou have set a home with the name §a${homeName} §bat§r: §a${Math.trunc(chatmsg.sender.location.x)}§r, §a${Math.trunc(chatmsg.sender.location.y)}§r, §a${Math.trunc(chatmsg.sender.location.z)}`, chatmsg.sender.nameTag);
        }
        else if (removeOptions.includes(args[0])) {
            if (!args[1])
                return Server.broadcast('§¶§cRebirth → §cPlease type a home name to remove!', chatmsg.sender.nameTag);
            if (!data.statusMessage.match(homeRegex))
                return Server.broadcast("§¶§cRebirth → §cYou don't have a home with that name!", chatmsg.sender.nameTag);
            else {
                Server.runCommand(`tag "${chatmsg.sender.nameTag}" remove "$(Home{Home-Name: ${homeName}, X: ${findXYZ[0]}, Y: ${findXYZ[1]}, Z: ${findXYZ[2]}})"`);
                return Server.broadcast(`§¶§cRebirth → §bSuccessfully removed home with the name §a${homeName} §bat §a${findXYZ[0]}§r, §a${findXYZ[1]}§r, §a${findXYZ[2]}`, chatmsg.sender.nameTag);
            }
            ;
        }
        else if (warpOptions.includes(args[0])) {
            if (!args[1])
                return Server.broadcast('§¶§cRebirth → §cPlease type a home name to warp to!', chatmsg.sender.nameTag);
            if (!data.statusMessage.match(homeRegex))
                return Server.broadcast("§¶§cRebirth → §cYou don't have a home with that name!", chatmsg.sender.nameTag);
            Server.runCommand(`execute "${chatmsg.sender.nameTag}" ~~~ tp @s ${findXYZ[0]} ${findXYZ[1]} ${findXYZ[2]}`);
            Server.runCommand( `playsound note.pling "${chatmsg.sender.nameTag}" ~ ~ ~` );
            return Server.broadcast(`§¶§cRebirth → §bYou have been teleported to §a${args[1]} §bat §a${findXYZ[0]}§r, §a${findXYZ[1]}§r, §a${findXYZ[2]}`, chatmsg.sender.nameTag);
        }
        else
            return Server.broadcast("§¶§cRebirth → §6USAGE §7: §b!home <list | set | remove | warp> [home name]", chatmsg.sender.nameTag);
    }
});
