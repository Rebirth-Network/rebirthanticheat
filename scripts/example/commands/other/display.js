...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    if( Server.player.getScore('icmtoggle', chatmsg.sender.nameTag) === 0) {
        ...
        return Server.broadcast(`§¶§cRebirth → §c§lRealm owner has set a global hotbar message `, chatmsg.sender.nameTag);
        ...
        if(personal.includes(args[0]))
        {
            ...
            Server.broadcast(`§l§¶§cRebirth → §b§lNow showing display for self stats `, chatmsg.sender.nameTag);
            Server.runCommand( `scoreboard players set "${chatmsg.sender.nameTag}" hometp 1337` );
        }
        else if(realm.includes(args[0])) {
            Server.runCommand( `playsound note.pling "${chatmsg.sender.nameTag}" ~ ~ ~` );
            Server.broadcast(`§l§¶§cRebirth → §b§lNow showing display for server stats `, chatmsg.sender.nameTag);
            Server.runCommand( `scoreboard players set "${chatmsg.sender.nameTag}" hometp 420` );
        }
        else if(off.includes(args[0])) {
            Server.runCommand( `playsound note.pling "${chatmsg.sender.nameTag}" ~ ~ ~` );
            Server.broadcast(`§l§¶§cRebirth → §b§lStats Display has been §cDISABLED `, chatmsg.sender.nameTag);
            Server.runCommand( `scoreboard players set "${chatmsg.sender.nameTag}" hometp 3` );
        }else {
            return Server.broadcast(`§¶§cRebirth → §cERROR! §6Usage Example §7:§b§l !display [ self | server | off ]`, chatmsg.sender.nameTag);
        }
    }else {
        return Server.broadcast(`§¶§cRebirth → §cERROR 2! §6Usage Example §7:§b§l !display [ self | server | off ]`, chatmsg.sender.nameTag);
    }
});