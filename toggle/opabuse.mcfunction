...
execute @s[tag=ownertag,scores={AOPAM=1}] ~~~ tellraw @a[tag=stafftag] {"rawtext":[{"text":"§¶§cRebirth §¶§b→ §6Anti OP Abuse §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
#turn off
...
execute @s[tag=ownertag,scores={AOPAM=2}] ~~~ tellraw @a[tag=stafftag] {"rawtext":[{"text":"§¶§cRebirth §¶§b→ §6Anti OP Abuse §bhas been toggled §cOFF §bby §d"},{"selector":"@s"}]}
execute @s[tag=ownertag,scores={AOPAM=2}] ~~~ scoreboard players set aopamtoggledummy AOPAM 0
#Deny Nonstaff
execute @s[tag=!ownertag] ~~~ tellraw @s {"rawtext":[{"text":"§¶§cRebirth §¶§b→ Access §cDENIED§7! §bOnly Ownerstatus can use this command"}]}
